...
CREATE TABLE TelemetryEvents(Id INTEGER PRIMARY KEY AUTO_INCREMENT, TelemetryId INTEGER NOT NULL, Value REAL NOT NULL, Type VARCHAR(32) NOT NULL, Date TIMESTAMP NOT NULL, FOREIGN KEY(TelemetryId) REFERENCES Telemetries(Id) ON DELETE CASCADE);

CREATE TABLE ExperienceTracks(Id INTEGER PRIMARY KEY AUTO_INCREMENT, ExperienceName VARCHAR(32) NOT NULL, TrackName VARCHAR(32) NOT NULL, FOREIGN KEY(ExperienceName) REFERENCES Experiences(Name), FOREIGN KEY(TrackName) REFERENCES Tracks(Name) ON DELETE CASCADE);
