...
CREATE TABLE Tracks(Id VARCHAR(36) PRIMARY KEY, Name VARCHAR(32) NOT NULL);

CREATE TABLE Checkpoints(Id INTEGER PRIMARY KEY AUTO_INCREMENT, TrackId VARCHAR(36) NOT NULL, Longitude REAL NOT NULL, Latitude REAL NOT NULL, Idx INTEGER NOT NULL, FOREIGN KEY(TrackId) REFERENCES Tracks(Id) ON DELETE CASCADE);

CREATE TABLE Telemetries(Id INTEGER PRIMARY KEY AUTO_INCREMENT, TrackId VARCHAR(36) NOT NULL, FOREIGN KEY(TrackId) REFERENCES Tracks(Id) ON DELETE CASCADE);
...
CREATE TABLE ExperienceTracks(Id INTEGER PRIMARY KEY AUTO_INCREMENT, ExperienceId VARCHAR(36) NOT NULL, TrackId VARCHAR(36) NOT NULL, FOREIGN KEY(ExperienceId) REFERENCES Experiences(Id), FOREIGN KEY(TrackId) REFERENCES Tracks(Id) ON DELETE CASCADE);

CREATE TABLE ExperienceUserPoints(Id INTEGER PRIMARY KEY AUTO_INCREMENT, ExperienceId VARCHAR(36) NOT NULL, Name VARCHAR(32) NOT NULL, Longitude REAL NOT NULL, Latitude REAL NOT NULL, FOREIGN KEY(ExperienceId) REFERENCES Experiences(Id), UNIQUE INDEX (ExperienceId, Latitude, Longitude));
...
CREATE TABLE SyncList(Id INTEGER PRIMARY KEY AUTO_INCREMENT, User VARCHAR(32) NOT NULL, ExperienceId VARCHAR(36) NOT NULL, FOREIGN KEY(ExperienceId) REFERENCES Experiences(Id) ON DELETE CASCADE, FOREIGN KEY(User) REFERENCES Users(Email));

CREATE TABLE TempTokens(Token VARCHAR(6) PRIMARY KEY, Date TIMESTAMP NOT NULL, DeviceId VARCHAR(32) UNIQUE);
